...
export default defineConfig({
  ...
  deps: {
    ...
    // dynamic `import()`s (code mode, PDF extraction, sharp, clipboard, …):
    // those are lazy loads, not optional installs, so they must ship inside the
    // bundle rather than resolve from node_modules.
    alwaysBundle: [/^@codent\//, /.*/],
  },
});
