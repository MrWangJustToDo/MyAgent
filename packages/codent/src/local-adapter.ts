...
/**
 * Local-only adapter. The release host has exactly one session plane — the
 * in-process manager (`createLocalAgentSessionHost`) — so there is no
 * `config.remoteSession` branch and therefore no `@codent/server` import.
 */
