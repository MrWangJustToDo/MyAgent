/**
 * Remote provider proxy — points TanStack adapters at a server that holds the
 ...
 * browser playground — can register a proxy-mode provider without depending on
 * `@my-agent/server`. Only uses `fetch` + core types, safe for Node and browsers.
 */

import type { ModelProvider } from "./model-provider.js";

/** Placeholder apiKey for provider proxy adapters (the server injects the real key). */
