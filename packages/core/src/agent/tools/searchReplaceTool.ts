...
export const createSearchReplaceTool = ({ sandbox }: { sandbox: Sandbox }) => {
  return tool({
    ...
    inputExamples: [
      { input: { path: "/src/index.ts", modifiedTime: "123", replacements: [{ oldString: "foo", newString: "bar" }] } },
    ],
    needsApproval: true,
    ...
    execute: async ({ path, modifiedTime, replacements }, { abortSignal }) => {
