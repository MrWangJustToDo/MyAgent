...
export const createEditFileTool = ({ sandbox }: { sandbox: Sandbox }) => {
  return tool({
    ...
    inputExamples: [
      { input: { path: "/src/index.ts", modifiedTime: "123", oldString: "foo", newString: "bar", replaceAll: false } },
    ],
    needsApproval: true,
    ...
    execute: async ({ path, modifiedTime, oldString, newString, replaceAll }, { abortSignal }) => {
