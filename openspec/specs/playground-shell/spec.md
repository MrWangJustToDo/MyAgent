...
The playground shell is the single host chrome that surrounds the terminal: a top bar, a work
area, and a status bar, built from one shared UI primitive layer and a layered stylesheet. It owns
the single responsive-breakpoint owner (compact / regular / wide) that decides between a resizable
side pane and an overlay sheet, the keyboard paths for primary actions (command palette, settings,
splitters), and the shared overlay dismissal contract used by `Dialog` and `Sheet`. The terminal
is sized so that resize and pane-drag events reflow it in place, without ever remounting the terminal
instance.
