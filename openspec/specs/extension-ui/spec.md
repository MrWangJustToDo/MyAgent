...
Extension-facing UI surface. Extensions draw through **one generic render outlet** — raw text (ANSI preserved) or a `text` / `row` / `column` / `box` layout tree — plus a transient notification path and a context snapshot. The host renders with a single generic renderer and there is deliberately no predefined extension component vocabulary; payloads stay JSON-serializable so any host (including remote ones) can render them.
