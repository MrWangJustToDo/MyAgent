...
- **`text` semantics**: `present.text` means "this rendered line *is* the row" — today's extension `toUI`. Built-in fallback descriptors therefore do **not** set it: `read_file` and friends fold into `1 read` today and must keep folding, and only a detailed built-in (`run_command`, `task`, …) renders a block. Stage 3 consequently puts **only** `present.text` into the payload: a built-in's block text comes from core's self-contained formatter switch, which the host runs itself, so filling `text` for built-ins would make every tool look like it owns a block (regression caught in review). `keepsCompactRow` keeps exactly today's meaning: `keepRow || clientSide || text`.
