...
`agent/tools/` MUST contain universal tool factories, the `createTools` assembler, `tool-config`, TanStack glue (`tanstack/`), shared tool helpers (`util/`), and websearch providers. Non-factory helpers that only support a single tool MUST NOT sit as peer files of `*-tool.ts` at the `tools/` root when they can live under `util/` or a feature subfolder.
