...
Give every tool a single, core-owned description of how it should be displayed (fold category, whether a completed row stays visible, whether it has a detailed result block, header summary text, activity labels, result text, input text), and ship the per-call result of that description to hosts as ordinary data. This removes the duplicated per-tool knowledge that hosts used to hard-code — which drifted repeatedly and broke whenever core ran outside the host process (remote CoreEnv, remote Agent Session, extension hosts).
