...
<!--
  `command-jobs` has no main spec yet: the change that introduced this capability
  (2026-08-12-run-command-background) was archived without merging its delta into
  openspec/specs/. The first five requirements below are restated verbatim in
  substance from that archived spec so archiving this change produces a complete
  main spec instead of a persistence-only one; the remaining requirements are new.
-->
