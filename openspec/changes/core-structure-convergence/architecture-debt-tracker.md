...
| ID | Item | Location | Status | Suggested direction |
|----|------|----------|--------|---------------------|
| P1-15 | `ManagedAgent` too large | `managed-agent.ts` | `[~]` | **Reopened 2026-09-24.** The `[x]` this row carried was true of a 514-line snapshot (the `RunCoordinator` extraction); the file then grew to ~1756 lines across ~129 methods behind an unexplained `/* eslint-disable max-lines */`. **Landed:** (a) accessor unification — 8 property getters removed, one spelling per concept, enforced by `validate:accessor-convention`; (b) `RunnerWiring` extracted (`managed-agent-runner-wiring.ts`) — runner cache + config key + text adapter + UI channel, with the rebind-detaches-approval-subscription rule, covered by `validate:runner-wiring`; (c) the layer-boundary gate generalized (see P1-18). **Remaining:** the 611-line "Config & resources" region and the 246-line "Status & events" region. Unlike (b) these are not a field move — they read and write the same object graph, so extracting them means moving policy and needs its own validator work. The `max-lines` disable now requires a written justification (AGENTS.md) but `managed-agent.ts` still needs the extracted shape before that can be written honestly. |
...
| P1-18 | Boundary gates covered 2 of 6 layer edges | `scripts/validate-{agent,models}-managers-boundary.mjs` | `[x]` | Both gates shared one regex (specifier contains `/managers/`), so `models → agent`, `runtime-types → *`, `env → agent` and `agent → agent-session` had **no** coverage. Replaced by `validate:layer-boundaries`: derives edges from the imports, checks them against a table where every entry carries a reason plus optional `onlySources`/`onlyTargets`, and adds a `type-only` mode so a runtime value crossing a type-only edge fails. Escapes it found and that are now fixed: `env → command-job-registry` (a real cycle, inverted via `env-teardown.ts` hooks), `agent/lsp/lsp-transport.ts` (port declared inside its consumer → `env-lsp.ts`), `runtime-types/agent-event-payloads → mcp/manager` (`McpServerStatus` → `runtime-types/mcp-status.ts`), `models/side-text-query → agent/agent-log` (a `SideQueryLog` port). Verified non-vacuous with probes for both the old rule and four edges the old gates were blind to. |
